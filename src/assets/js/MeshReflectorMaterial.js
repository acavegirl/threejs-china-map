...
export default class MeshReflectorMaterial extends MeshStandardMaterial {
  ...
  beforeRender() {
    if (!this.parent || !this.camera.matrixWorld || !this.gl.xr) return
    ...
    if (!projectionMatrix.elements) projectionMatrix.elements = []
  ...
  update() {
    if (this.parent.material !== this || !this.gl.xr) return;
